...
const addressOption = Options.text('address').pipe(Options.optional, Options.withDescription('Any Fast address (fast1...) to query'));
...
export const infoBalance = Command.make('balance', { address: addressOption, token: tokenOption }, (args) =>
  Effect.gen(function* () {
      ...
      if (tokenBalances) {
        for (const [tokenId, amount] of Object.entries(tokenBalances)) {
          ...
          if (Option.isSome(args.token)) {
            ...
            if (tokenId !== args.token.value && !tokenId.toLowerCase().includes(filter)) {
              continue;
            }
          }
