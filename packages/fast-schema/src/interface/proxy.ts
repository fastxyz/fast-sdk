...
export const makeFaucetDripParams = <
  TAddr extends S,
  TAmt extends S,
  TId extends S,
>(p: {
...
export const makeGetAccountInfoParams = <
  TAddr extends S,
  TId extends S,
  TKey extends S,
  TNonce extends S,
>(p: {
  ...
  CamelCaseStruct({
    ...
    certificate_by_nonce: Schema.NullOr(
      Schema.Struct({ start: p.Nonce, limit: Schema.Number }),
    ),
  });

export const makeGetPendingMultisigParams = <TAddr extends S>(p: {
  Address: TAddr;
}) => Schema.Struct({ address: p.Address });

export const makeGetTokenInfoParams = <TId extends S>(p: { TokenId: TId }) =>
  CamelCaseStruct({ token_ids: Schema.Array(p.TokenId) });

export const makeGetTransactionCertificatesParams = <
  TAddr extends S,
  TNonce extends S,
>(p: {
  Address: TAddr;
...
export const GetAccountInfoInput = Schema.Struct({
  ...
  tokenBalancesFilter: Schema.optionalWith(
    Schema.NullOr(Schema.Array(TokenIdFromInput)),
    { default: () => null },
  ),
  stateKeyFilter: Schema.optionalWith(
    Schema.NullOr(Schema.Array(StateKeyFromInput)),
    { default: () => null },
  ),
...
export type GetPendingMultisigInputParams =
  typeof GetPendingMultisigInput.Encoded;
export type GetTokenInfoInputParams = typeof GetTokenInfoInput.Encoded;
export type GetTransactionCertificatesInputParams =
  typeof GetTransactionCertificatesInput.Encoded;
