 ...
 */
export const HexBigInt = Schema.transform(
  Schema.String,
...
export const UintNumber = <N extends number>(bits: N) =>
  Schema.Number.pipe(
    Schema.int(),
    Schema.between(0, 2 ** bits - 1),
    Schema.brand(`Uint${bits}` as `Uint${N}`),
  );
...
export const IntNumber = <N extends number>(bits: N) =>
  Schema.Number.pipe(
    Schema.int(),
    Schema.between(-(2 ** (bits - 1)), 2 ** (bits - 1) - 1),
    Schema.brand(`Int${bits}` as `Int${N}`),
  );
...
export const UintBigInt = <N extends number>(bits: N) =>
  Schema.BigIntFromSelf.pipe(
    Schema.betweenBigInt(0n, 2n ** BigInt(bits) - 1n),
    Schema.brand(`Uint${bits}` as `Uint${N}`),
  );
...
export const IntBigInt = <N extends number>(bits: N) =>
  Schema.BigIntFromSelf.pipe(
    Schema.betweenBigInt(-(2n ** BigInt(bits - 1)), 2n ** BigInt(bits - 1) - 1n),
    Schema.brand(`Int${bits}` as `Int${N}`),
  );

/** Hex string to branded unsigned bigint. */
export const HexUintBigInt = <N extends number>(bits: N) =>
  Schema.compose(HexBigInt, UintBigInt(bits));

/** Hex string to branded signed bigint. */
export const HexIntBigInt = <N extends number>(bits: N) =>
  Schema.compose(HexBigInt, IntBigInt(bits));

/** Decimal string to branded unsigned bigint. */
export const DecimalUintBigInt = <N extends number>(bits: N) =>
  Schema.compose(DecimalBigInt, UintBigInt(bits));

/** Decimal string to branded signed bigint. */
export const DecimalIntBigInt = <N extends number>(bits: N) =>
  Schema.compose(DecimalBigInt, IntBigInt(bits));
