...
/**
 * Fast network IDs used in signed transactions.
 * Defined locally since FAST_NETWORK_IDS no longer exists in fast-sdk.
 */
export const FAST_NETWORK_IDS: Record<string, string> = {
  "fast-testnet": "fast:testnet",
  "fast-mainnet": "fast:mainnet",
};

// BCS types from fast-schema
...
/**
 * Serialize a Fast transaction using BCS.
 */
...
/**
 * Serialize a transaction as a VersionedTransaction (wraps in Release20260319).
 */
...
/**
 * Decode a transaction envelope from BCS bytes.
 */
export function decodeEnvelope(
  ...
  // Unwrap the versioned transaction
...
export function getTransferDetails(decoded: DecodedFastTransaction): TransferDetails | null {
  ...
  // Batch with single TokenTransfer
  ...
  const amount = typeof transfer.amount === 'bigint' 
    ? transfer.amount 
