...
export const rootCommand = Command.make('fast', rootOptions).pipe(
  Command.withDescription('Fast CLI - Account, network, and transaction management'),
  Command.provide(makeAppLayer),
  Command.withSubcommands([accountCommand, networkCommand, infoCommand, sendCommand]),
);
