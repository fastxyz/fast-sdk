...
const nameOption = Options.text("name").pipe(
  Options.optional,
  Options.withDescription("Alias for the account"),
)

const privateKeyOption = Options.text("private-key").pipe(
  Options.optional,
  Options.withDescription("Hex-encoded Ed25519 seed (0x-prefixed or raw)"),
)

const keyFileOption = Options.text("key-file").pipe(
  Options.optional,
  Options.withDescription("Path to a JSON file containing a privateKey field"),
)
...
export const accountImport = Command.make(
  "import",
  { name: nameOption, privateKey: privateKeyOption, keyFile: keyFileOption },
  (args) =>
    Effect.gen(function* () {
      ...
      if (Option.isSome(args.privateKey) && Option.isSome(args.keyFile)) {
        return yield* Effect.fail(
          new InvalidUsageError({ message: "--private-key and --key-file are mutually exclusive" }),
        )
      }
      ...
      if (Option.isSome(args.privateKey)) {
        ...
        if (seed.length !== 32) {
          return yield* Effect.fail(
            new InvalidUsageError({ message: "Private key must be exactly 32 bytes (64 hex characters)" }),
          )
        }
        ...
        if (!parsed.privateKey) {
          return yield* Effect.fail(
            new InvalidUsageError({ message: "Key file must contain a 'privateKey' field" }),
          )
        }
        ...
        return yield* Effect.fail(
          new InvalidUsageError({ message: "Provide --private-key or --key-file" }),
        )
      }
