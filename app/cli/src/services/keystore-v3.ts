...
export interface KeystoreV3Shape {
  readonly encrypt: (seed: Uint8Array, password: string, fastAddress: string, evmAddress: string) => Effect.Effect<KeyfileV3>;
  readonly decrypt: (keyfile: KeyfileV3, password: string) => Effect.Effect<Uint8Array, WrongPasswordError>;
}

export class KeystoreV3 extends Context.Tag('KeystoreV3')<KeystoreV3, KeystoreV3Shape>() {}
  ...
  encrypt: (seed, password, fastAddress, evmAddress) =>
    Effect.tryPromise({
      try: async () => {
        ...
        const derivedKey = await scryptAsync(new TextEncoder().encode(password), salt, { N: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P, dkLen: DKLEN });
