...
describe('FastProvider integration (real RPC)', () => {
  it.runIf(hasRpc && hasSigner)('getAccountInfo returns validated account payload', async () => {
  ...
  it.runIf(hasRpc && hasSigner)('getPendingMultisigTransactions returns an array', async () => {
    const signer = new Signer(env.signerPrivateKey!);
    const pubKey = await signer.getPublicKey();
    const provider = new FastProvider({ rpcUrl: env.rpcUrl! });
  ...
  it.runIf(hasRpc && hasSigner)('getTransactionCertificates returns an array', async () => {
    const signer = new Signer(env.signerPrivateKey!);
    const pubKey = await signer.getPublicKey();
    const provider = new FastProvider({ rpcUrl: env.rpcUrl! });
  ...
  it.runIf(hasRpc && hasSigner)('getTokenInfo returns nullable token metadata response', async () => {
    const signer = new Signer(env.signerPrivateKey!);
    const pubKey = await signer.getPublicKey();
    const provider = new FastProvider({ rpcUrl: env.rpcUrl! });
  ...
  it.runIf(hasRpc && hasSigner)('faucetDrip is reachable with signer-derived recipient', async () => {
    const signer = new Signer(env.signerPrivateKey!);
    const pubKey = await signer.getPublicKey();
    const provider = new FastProvider({ rpcUrl: env.rpcUrl! });
  ...
  it.runIf(hasRpc && hasSigner)('submitTransaction full pipeline: getAccountInfo → build → sign → submit', async () => {
    const provider = new FastProvider({ rpcUrl: env.rpcUrl! });
    const signer = new Signer(env.signerPrivateKey!);
    const pubKey = await signer.getPublicKey();
