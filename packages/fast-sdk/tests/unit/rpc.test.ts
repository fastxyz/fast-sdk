...
describe('rpcCallEffect', () => {
  ...
  it('sends correct JSON-RPC request format', async () => {
    const fetchMock = vi.fn(() =>
      Promise.resolve({
        text: () => Promise.resolve(JSON.stringify({ jsonrpc: '2.0', id: 1, result: 'ok' })),
      }),
    );
  ...
  it('throws on network failure', async () => {
    ...
    await expect(run(rpcCallEffect('http://localhost', 'test', {}))).rejects.toThrow();
  });
  ...
  it('handles bigint values in response', async () => {
    ...
    const result = (await run(rpcCallEffect('http://localhost', 'test', {}))) as { balance: number | bigint };
