...
describe('send recipient validation', () => {
  it('maps a malformed fast-prefixed recipient to InvalidAddressError', async () => {
    ...
    const exit = await Effect.runPromiseExit(
      send.handler({ address: 'fast1bad', amount: '1', token: 'testUSDC', replacePending: false } as never).pipe(Effect.provide(layer)),
    );
  ...
  it('rejects a Fast recipient with a non-32-byte payload before resolving the account', async () => {
    const seed = new Uint8Array(32).fill(1);
    const signer = new Signer(seed);
    const account: AccountInfo = {
      kind: 'single',
    ...
    const invalidRecipient = bech32m.encode('fast', bech32m.toWords(new Uint8Array(31)));
    const layer = Layer.mergeAll(
      Layer.succeed(AccountStore, {
        resolveAccount: () => Effect.die('account resolution must not run'),
        export: () => Effect.succeed({ seed, account }),
        list: () => Effect.succeed([account]),
      } as never),
      Layer.succeed(AllSet, {} as never),
    ...
    const exit = await Effect.runPromiseExit(
      send.handler({ address: invalidRecipient, amount: '1', token: 'testUSDC', replacePending: false } as never).pipe(Effect.provide(layer)),
    );
