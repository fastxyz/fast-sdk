...
import { generateEd25519Key, saveKeyfile, loadKeyfile, withKey, signEd25519, verifyEd25519, } from './keys.js';
...
import { TransactionBcs, hashTransaction, FAST_DECIMALS, SET_TOKEN_ID, EXPLORER_BASE, tokenIdEquals, hexToTokenId, } from './bcs.js';
...
function isNativeFastToken(token) {
...
export function fast(opts) {
    ...
    async function fetchAccountInfo(pubkey) {
    ...
    function buildTokenBalanceMap(accountInfo) {
        const map = new Map();
    ...
    async function fetchTokenMetadata(tokenIds) {
        const uniq = new Map();
    ...
    async function resolveNamedToken(accountInfo, token) {
        const upper = token.toUpperCase();
    ...
    const client = {
        ...
        async submit(params) {
            ...
            try {
                return await withKey(_keyfilePath, async (keypair) => {
                    const accountInfo = (await fetchAccountInfo(senderPubkey));
