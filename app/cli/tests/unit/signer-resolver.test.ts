...
describe('resolveSigner', () => {
  ...
  it('fails NotAMemberError when --as names a non-member account', async () => {
    const layer = makeLayer();
    const aliceAddr = await fastAddressOf(SEED(0xaa));
    const bobAddr = await fastAddressOf(SEED(0xbb));
    ...
    const exit = await Effect.runPromiseExit(
      Effect.gen(function* () {
        const accounts = yield* AccountStore;
        yield* accounts.create('alice', SEED(0xaa), null);
        yield* accounts.create('mallory', SEED(0xdd), null);
        const ms = yield* accounts.createMultiSig(
          {
            version: 1,
            name: 'treasury',
    ...
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        return yield* resolveSigner({
          ...
          passwordFor: (member) => {
            passwordRequests++;
            return Effect.succeed(member.encrypted ? 'secret' : null);
          },
        });
      }).pipe(Effect.provide(layer)),
    );
