...
export const Uint8ArrayFromNumberArray = Schema.transform(
...
export const Uint8ArrayFromHexOptional0x = Schema.compose(
	Strip0xHex,
	Uint8ArrayFromHex,
);
	...
	Schema.transformOrFail(Schema.String, Schema.Uint8ArrayFromSelf, {
		...
		encode: (data, _options, ast) => {
			...
			try {
				...
				const failure = new ParseResult.Transformation(
					ast,
					data,
					"Transformation",
					inner,
...
export const FixedUint8ArrayFromNumberArray = <N extends number>(n: N) =>
	Schema.compose(Uint8ArrayFromNumberArray, FixedUint8Array(n));

/** Decodes a hex string into a fixed-length `Uint8Array(n)` and encodes back. */
export const FixedUint8ArrayFromHex = <N extends number>(n: N) =>
	Schema.compose(Uint8ArrayFromHex, FixedUint8Array(n));

/** Decodes a hex string (with optional `0x` prefix) into a fixed-length `Uint8Array(n)`. */
export const FixedUint8ArrayFromHexOptional0x = <N extends number>(n: N) =>
	Schema.compose(Uint8ArrayFromHexOptional0x, FixedUint8Array(n));
