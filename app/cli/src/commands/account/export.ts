...
const nameArg = Args.text({ name: "name" }).pipe(
  Args.withDescription("Account alias. Defaults to the default account."),
  Args.optional,
)
...
export const accountExport = Command.make("export", { name: nameArg }, (args) =>
  Effect.gen(function* () {
    ...
    const accountName = Option.isSome(args.name)
      ? args.name.value
      : (yield* accounts.resolveAccount(config.account)).name
