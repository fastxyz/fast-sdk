...
export const submitTransaction = (rpcUrl: string, params: TransactionEnvelope) =>
  Effect.gen(function* () {
    const wire = yield* Schema.encode(SubmitTransactionParamsFromRpc)(params);
    const result = yield* rpcCallEffect(rpcUrl, 'proxy_submitTransaction', wire);
    return yield* Schema.decodeUnknown(ProxySubmitTransactionResultFromRpc)(result);
  });
...
export const getPendingMultisigTransactions = (rpcUrl: string, params: GetPendingMultisigParams) =>
  Effect.gen(function* () {
    const wire = yield* Schema.encode(GetPendingMultisigParamsFromRpc)(params);
    const result = yield* rpcCallEffect(rpcUrl, 'proxy_getPendingMultisigTransactions', wire);
    return yield* Schema.decodeUnknown(Schema.Array(TransactionEnvelopeFromRpc))(result);
  });
...
export const getTransactionCertificates = (rpcUrl: string, params: GetTransactionCertificatesParams) =>
  Effect.gen(function* () {
    const wire = yield* Schema.encode(GetTransactionCertificatesParamsFromRpc)(params);
    const result = yield* rpcCallEffect(rpcUrl, 'proxy_getTransactionCertificates', wire);
    return yield* Schema.decodeUnknown(Schema.Array(TransactionCertificateFromRpc))(result);
  });
