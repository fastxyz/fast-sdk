...
import { getPublicKey, signMessage, signTypedData, verify, verifyTypedData } from '../../src/core/crypto/signing';
...
describe('verify', () => {
  ...
  it('returns false for tampered message', async () => {
    ...
    const pub = await run(getPublicKey(PRIV_KEY));
    const sig = await run(signMessage(PRIV_KEY, msg));
    const valid = await run(verify(sig, new TextEncoder().encode('tampered'), pub));
